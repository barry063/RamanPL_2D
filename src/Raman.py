...
class Raman:
    ...
    def smooth_data(self, window_size=5, data='Intensity', base = 0):
        """
        Smooth Raman spectrum data using a moving average and linearly interpolate NaN values.
    ...
    def normalise(self, data="Intensity", max=None):
        """
        Normalize the Raman spectrum data.
    ...
    def curve_fit(self, fitting_type='Lorenztian', data='Intensity', initial_guesses=None, num_peaks=1, plot=True):
        """
        Perform curve fitting on Raman spectrum data with multiple peaks.

        Parameters:
        - fitting_type (str, optional): The type of fitting to perform ('Lorenztian', 'Gauss', 'Voigt').
        ...
        valid_fitting_types = ['Lorenztian', 'Gauss', 'Voigt']
        ...
        self.fitted_data = pd.DataFrame({'Wavenumber': x, 'Intensity': fitted_data})
    ...
    def plot_spectrum(self, data='Intensity', indicate_peaks=True, fitted_plot=True,
        ...
        if fitted_plot:
            if self.fitted_data is not None:
                fitted_x = self.fitted_data['Wavenumber']
        ...
        if indicate_peaks and self.peak_values is not None:
            ...
            peak_x = self.peak_values['Wavenumber']
