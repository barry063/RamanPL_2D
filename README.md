...
| v0.6.7 ✓ | Integrated component area in the feature table | Shipped 2026-06-05. Per-peak `{peak}_component_area`, `{peak}_component_area_norm`, and `{peak}_component_area_fraction` columns added to `feature_table()` for all fitter classes. Values are exact analytic areas derived from the `amp` fit parameter (no numerical integration). New `area_ratios=` keyword adds `{P1}_{P2}_area_ratio` columns. Additive-only: existing columns byte-identical; frozen suffix vocabulary extended in `api-stability.md` §14. No changes to fitting, preprocessing, baseline algorithms, or export schemas. |
| v0.6.8 shipped | Visualisation module extraction | Shipped 2026-06-22. v0.6.8 extracts fitted mapping, single-fit, and batch plotting implementations into the internal `ramanpl.visualisation` package. Existing class methods and `ramanpl.batch` plotting imports remain supported with unchanged public signatures and return contracts. `ramanpl.visualisation` remains internal until v0.7.0. Integration-class plotting remains unchanged and is deferred for later cleanup. No fitting, preprocessing, baseline, export, or feature-table schema changes. |
