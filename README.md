...
| **v0.5.5** | Consolidation pause: API freeze, docs, dependency hygiene | - Lock down the public surface introduced in v0.5.1–v0.5.4 as a written, testable contract: feature-table column schema, suffix vocabulary (`_position`, `_fwhm`, `_peak_height`, `_peak_height_norm`, `_separation`, `_ratio`), QA column names (`rmse`, `ok`, `n_starts`, `n_params_at_bounds`), descriptor naming, and the `[ml]` extra boundary. <br> - Add `docs/source/api-stability.md` (frozen-surface reference) and `tests/test_api_stability.py` enforcing an additive-only column rule. <br> - Document the ratio/separation order convention and confirm modality parity (the same column schema applies uniformly to Raman and PL mapping) across the user guide. <br> - Consolidate user-guide pages and the changelog around the v0.5.1–v0.5.4 exports and `feature_table()` accessor (no new exports introduced in this build). <br> - Expose `ramanpl.descriptors` at the top level for consistency with its API-reference status. <br> - Confirm via CI that the base install does not require scikit-learn and that `ramanpl.ml` raises a clean `ImportError` when the extra is missing. <br> - No new algorithms in this build. |
