...
| **v0.5.5** | Consolidation pause: API freeze, docs, dependency hygiene | - Added `docs/source/api-stability.md`: written, citable freeze contract for the public surface introduced in v0.5.1–v0.5.4; pins suffix vocabulary, QA column names, all six `feature_table()` methods, descriptor functions, `pca_reduce`, `kmeans_cluster`, and the `[ml]` boundary. <br> - Added `tests/test_api_stability.py` (8 tests): enforces the additive-only column rule and frozen vocabularies as a regression test. <br> - Added order-convention and PL-parity documentation across mapping/batch/single-spectrum/clustering user-guide pages; added cross-links between user-guide pages and the clustering page. <br> - Added `[ml]` install option to `README.md` and `docs/source/installation.md` with explicit base-install guarantee. <br> - Exposed `ramanpl.descriptors` at the top level (`from ramanpl import descriptors` now works); `__all__` and `_MODULE_EXPORTS` updated. <br> - Extended `tests/test_notebook_smoke.py` with three new notebooks (`Feature_Table_Example`, `Peak_Proposal_Demo`, `Clustering_Demo`); CI notebook-smoke now installs `[ramanspy,ml]`. <br> - 13 new tests added (3 order-convention in `test_descriptors_unit.py`, 8 freeze regression in `test_api_stability.py`, 1 in `test_feature_table_mapping.py`, 1 in `test_packaging_smoke.py`). <br> - No new algorithms; no changes to fitting, preprocessing, or export schemas. |
| **v0.6.0** | Stable interpretable-analysis milestone | - Declared the first stable interpretable-analysis milestone built on v0.5.1–v0.5.5. <br> - Stable components: per-pixel QA-augmented mapping export, generalised peak descriptors, `feature_table()` accessors, classical peak-proposal aid for failed-fit recovery, optional unsupervised clustering on fitted descriptors. <br> - Added `benchmarks/validation_v0.6.0_vs_v0.5.0.py`: reproducible validation harness comparing fit quality, runtime, and failure-mode behaviour against the v0.5.0 baseline on four synthetic cubes; fit-quality parity confirmed. <br> - Added `example-usage/Validation/Validation_v0.6.0_vs_v0.5.0.ipynb`: executable validation notebook (added to `test_notebook_smoke.py`). <br> - Added `docs/source/validation/v0.6.0.md`: citable static validation report. <br> - Updated `docs/source/api-stability.md` scope to v0.5.5–v0.6.0; four frozen surfaces unchanged. <br> - Deterministic physical fitting (Lorentzian / pseudo-Voigt least squares) remains the final authority for all reported peak parameters. <br> - Supervised classification remains deferred to v0.7.x, conditional on curated, labelled, multi-instrument datasets. |
