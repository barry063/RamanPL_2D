...
Post-v0.6.0 development honours the v0.5.5–v0.6.0 API freeze and continues to treat deterministic Lorentzian / pseudo-Voigt least-squares fitting as the final authority for reported peak parameters. The v0.6.x series targets three concrete pain points raised after real mapping use: lack of progress feedback on long fits, manually-tuned preprocessing parameters, and single-threaded mapping runtime on multi-core machines. v0.6.5 is a consolidation pause mirroring the v0.5.5 pattern, freezing the new additive surface before v0.6.6 begins separating plotting from fitting classes. Full deprecation of the embedded plotting methods, and promotion of `ramanpl.visualisation` to a stable public API, is deferred to v0.7.x.

| Version | Scope | Details |
|---|---|---|
| v0.6.1 | Progress indicators for mapping and batch fits | - Add `tqdm` as a hard runtime dependency in `pyproject.toml`.<br>- Wrap the pixel loop in `RamanMapping.fit_spectra` and `PLMapping.fit_spectra` with a `tqdm` progress bar reporting pixels processed and ETA; throttled with `mininterval=0.5`. Same for `RamanBatch.fit` and `PLBatch.fit` file loops.<br>- Expose `show_progress=True` keyword (default `True`) so the bar can be disabled if needed.<br>- Add a packaging-smoke test verifying `tqdm` is importable from the installed package.<br>- Measure tqdm overhead against `benchmark_mapping_fit.py` and record in the closeout note. Expected impact < 0.1 % of total runtime.<br>- No changes to fitting algorithms, fitted values, or export schemas. |
| v0.6.2 | Seed-pixel baseline auto-tuning with visual review | - Add an opt-in `autotune_baseline(seed_coord=..., methods=..., lam_grid=..., plot=True)` method on `RamanMapping` and `PLMapping` running a small grid over baseline configurations on a single seed pixel.<br>- Each candidate scored by post-fit RMSE on the seed pixel; full ranking returned, not just the winner.<br>- When `plot=True`, produce a comparison figure overlaying each candidate's preprocessed spectrum with annotated RMSE, reusing existing single-point plotting primitives.<br>- Apply the chosen configuration cube-wide only when the user explicitly accepts it (`apply_choice(...)` or by passing the choice back to `fit_spectra`).<br>- Record the grid, scores, and winner in export metadata under the existing provenance system.<br>- Never run silently during normal `fit_spectra` calls — must be invoked explicitly.<br>- No changes to baseline algorithms in `baselineAPI.py`. |
| v0.6.3 | Parallel mapping fit with tile-based warm-start | - Add `n_jobs` keyword to `fit_spectra` (default `1` = serial, preserving current behaviour).<br>- When `n_jobs > 1`, split the cube into `n_jobs` rectangular tiles; each tile runs its own warm-start chain in a `joblib` worker. Tile results aggregated back into `fitted_params`, `residual_map`, `peak_intensities`, and `fit_diagnostics_map`.<br>- Acceptance criteria: (i) byte-parity vs the current serial path when `n_jobs=1`; (ii) `n_curve_fit_calls` identical regardless of `n_jobs` on the same cube; (iii) measurable wall-clock reduction at `n_jobs > 1` on the existing benchmark.<br>- Extend `benchmarks/benchmark_mapping_fit.py` with an `n_jobs` axis. Add `joblib` as a hard dependency.<br>- No changes to per-pixel fitting algorithms or scientific output values. |
| v0.6.4 | Similarity-based seed selection for warm-start | - Add an internal helper that computes a cheap PCA + k-means partition of the preprocessed cube; controlled by a new `cluster_seeds` keyword on `fit_spectra` (default off, preserving current behaviour).<br>- When enabled, each cluster's representative pixel is fitted first; remaining pixels in that cluster receive its result as their warm-start initial guess.<br>- Compatible with v0.6.3 tiling: clusters honoured first, parallelism applied within them where independent.<br>- Acceptance criterion: reduction in `n_curve_fit_calls` on the existing benchmark, with per-pixel fitted parameters within a documented tolerance of the v0.6.3 result.<br>- Requires the `[ml]` extra (already optional from v0.5.4); gracefully falls back to serial warm-start when `cluster_seeds` is requested but `[ml]` is not installed. |
| v0.6.5 | Consolidation pause: API freeze, validation, docs | - Update `docs/source/api-stability.md` to cover the v0.6.1–v0.6.4 additive surface: `show_progress` keyword, `autotune_baseline` method and its return object, `n_jobs` keyword and parallel-fit contract (byte-parity at `n_jobs=1`, `n_curve_fit_calls` invariance), `cluster_seeds` keyword.<br>- Add regression tests to `tests/test_api_stability.py` enforcing the new contract.<br>- Add `benchmarks/validation_v0.6.5_vs_v0.6.0.py` and a citable `docs/source/validation/v0.6.5.md` report, mirroring the v0.6.0 validation pattern.<br>- Documentation pass: update `mapping.md`, `batch.md`, and `low_snr_advisory.md` for the new options.<br>- No new algorithms; no changes to fitting, preprocessing, or export schemas. |
| v0.6.6 | Visualisation module extraction | - Create `src/ramanpl/visualisation/` and move the implementation of `plot_heatmap`, `plot_ratio_heatmap`, `plot_residual_distribution`, `plot_spectrum`, `plot_spectrum_fit`, `plot_waterfall`, `plot_overlay`, and `plot_parameters` into it.<br>- Keep the existing class methods on `RamanMapping`, `PLMapping`, `RamanFit`, `PLfit`, `RamanBatch`, `PLBatch` as thin delegating façades — public method signatures unchanged. `ramanpl.visualisation` itself remains internal in this build.<br>- Add a small standalone preview helper (`ramanpl.visualisation._plot_raw_spectrum`) for quick pre-fit inspection of a single pixel or file.<br>- Acceptance criterion: notebook-smoke output figures unchanged versus v0.6.5 on the same inputs.<br>- Promotion of `ramanpl.visualisation` to a stable public surface, and deprecation of the embedded class methods, is deferred to v0.7.0. |
