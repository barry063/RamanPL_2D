...
| v0.6.3 | Autotune API refinement, real-data notebooks, API docs | - Replace `methods` + `lam_grid` with a `method_grids` dict that specifies per-method parameter sweeps and generates candidates via Cartesian product (e.g. `{"arpls": {"lam": [1e4,1e5], "niter": [50,100]}}`). Keep `methods`/`lam_grid` as `DeprecationWarning` shims for one version.<br>- Extend `Baseline_Autotune_Demo.ipynb` with a real-data section showing autotune on a spectrum with a genuine non-linear background; add code blocks to `example-usage/Ramanfit/Raman_background-remove.ipynb`.<br>- Add Sphinx autodoc entries for `autotune_baseline`, `apply_choice`, and `BaselineAutotuneResult` in `docs/source/api/`.<br>- Update all 19 autotune test call sites from `methods=[...]` to `method_grids={...}` syntax.<br>- No changes to fitting algorithms, `apply_choice` semantics, or export schemas. |
